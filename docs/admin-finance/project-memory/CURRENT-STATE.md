...
- Restore-drill runner теперь до снятия backup сверяет точный отсортированный
  список `schema.table` source-БД со списком 47 таблиц из baseline, чей checksum
  закреплён migration manifest. Одного совпадения количества недостаточно.
- Неполная или иная source-схема блокируется сразу после первого read-only
  list query; `pg_dump` и последующие restore-шаги не запускаются. После restore
  точный список таблиц повторно сравнивается с source и baseline.
- Добавлены негативные regression tests на source с 19 таблицами, source с 47
  таблицами и одним неверным именем, а также несовпадающую restored-схему. Полный
