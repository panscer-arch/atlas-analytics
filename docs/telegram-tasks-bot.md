...
Голос бота: он общается от имени `SuperSUS` — дружелюбно, чуть комично и церемонно, но коротко и по делу. В интерактивных командах и ATL/YouTrack-уведомлениях он пишет как живой операционный участник: что двигается, что ушло на тест, где нужен ответ и что надо подпнуть. Чистый endpoint `/api/telegram/send-message` отправляет текст как есть, без автоперсонажа.
...
Шутливый reply на вопрос SuperSUS про категорию:

```text
в угол
```

Бот не создаёт задачу, ставит лёгкую реакцию и отвечает шуткой в reply.

Реакция `💋`:

1. Написать обычное сообщение с задачей в чат.
2. Поставить на него реакцию `💋`.
3. Бот спросит: `SuperSUS: задача поймана. Куда кладём, мой друг?`
...
Автоматические ATL/YouTrack-уведомления идут в живом стиле `SuperSUS на связи`: короткий человеческий вывод сверху, затем конкретные задачи и ссылки. Формат должен оставаться операторским, без служебных полей вроде `статус отправлено`, `приоритет` и без упоминания Codex.
