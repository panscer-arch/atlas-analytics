...
3. Бот спросит: `Куда поставить задачу?`
...
Для обычных текстовых сообщений в Telegram использовать `/api/telegram/send-message`. Не использовать `/api/telegram/push-subtask` для ручных сводок: он предназначен для служебного push подзадач и добавляет поля ответственного, статуса и приоритета.
