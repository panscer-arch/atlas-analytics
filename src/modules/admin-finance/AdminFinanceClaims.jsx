...
const statusTabs = [
  ["all", "Все", 186], ["eligible", "Eligible", 42], ["requested", "Requested", 18], ["pending", "Pending", 9],
  ["failed", "Failed", 3], ["paid", "Paid", 112], ["reversed", "Reversed", 2],
];
const statusTone = { eligible: "orange", requested: "blue", pending: "violet", failed: "red", paid: "green", reversed: "red", expired: "brown" };
const componentLabels = {
  principal: "Principal",
  delta: "Gross Delta",
  partner_reward_creation: "Partner Reward at Creation",
  partner_reward_streamed: "Partner Reward streamed",
};
...
function ClaimsMethodology({ onClose }) {
  return <div className="af-modal-layer" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="af-modal" role="dialog" aria-modal="true" aria-labelledby="af-claims-method-title"><div className="af-modal-head"><div><span><BookOpenCheck size={17} /> Методика</span><h2 id="af-claims-method-title">Состояния claim и признание выплаты</h2></div><button type="button" onClick={onClose} aria-label="Закрыть"><X size={19} /></button></div><dl><div><dt>Eligible</dt><dd>Обязательство доступно к запросу, но фактического cash-out ещё нет.</dd></div><div><dt>Requested</dt><dd>Пользователь отправил запрос; транзакция ещё не подтверждена.</dd></div><div><dt>Pending</dt><dd>Транзакция создана или отправлена и ожидает финального результата.</dd></div><div><dt>Paid</dt><dd>Выплата признаётся только после подтверждённого settled/paid результата.</dd></div><div><dt>Failed</dt><dd>Остаётся обязательством до retry, подтверждённого reversal или expiry.</dd></div></dl><p className="af-modal-warning"><Info size={16} /> Maximum Eligible Exposure не равно ожидаемому cash-out: для Expected нужна проверенная модель claim-delay.</p></section></div>;
}
...
function ClaimsRequestState({ request }) {
  const title = request.status === "loading"
    ...
    : request.status === "auth-required"
      ? "Нужна админ-сессия"
      : "Реестр claims недоступен";
  const copy = request.status === "loading"
    ? "Получаем обязательства и payout graph из Admin API."
    : request.status === "auth-required"
      ? "API вернул 401. Макетные суммы не подставлены."
      : "Запрос не выполнен. Последние demo-значения намеренно скрыты.";
  return <section className="af-api-boundary" aria-live="polite"><AlertCircle size={24} /><span>CLAIM LIFECYCLE · FAIL-CLOSED</span><h2>{title}</h2><p>{copy}</p>{request.status !== "loading" ? <button type="button" onClick={request.reload}>Повторить запрос</button> : null}<a href="/admin/methodology#gate">Проверить источники и Gate 0</a></section>;
}

function ApiClaimDetail({ request }) {
  if (!request || request.status === "loading") return <aside className="af-panel af-claim-detail af-claim-empty"><div><strong>Загрузка claim</strong><p>Получаем payout components и lifecycle timestamps.</p></div></aside>;
  if (request.status === "ready" && !request.data) return <aside className="af-panel af-claim-detail af-claim-empty"><div><strong>В выбранном срезе нет claims</strong><p>Карточка появится после получения первой записи из канонического источника.</p></div></aside>;
  if (request.status !== "ready") return <aside className="af-panel af-claim-detail af-claim-empty"><div><strong>Claim недоступен</strong><p>Детали не подменяются данными из строки реестра.</p><button className="af-small-link" type="button" onClick={request.reload}>Повторить</button></div></aside>;
  ...
  return <aside className="af-panel af-claim-detail"><PanelHeader title={shortId(claim.id)} subtitle={`Participant ${shortId(claim.participantId)}`} action={<Tag tone={statusTone[claim.status]}>{claim.status.toUpperCase()}</Tag>} /><div className="af-claim-detail-body"><dl><div><dt>Claim ID</dt><dd className="af-lineage">{shortId(claim.id)}</dd></div><div><dt>Cycle ID</dt><dd className="af-lineage">{shortId(claim.cycleId)}</dd></div><div><dt>Eligible at · UTC</dt><dd>{formatUtc(claim.eligibleAt)}</dd></div><div><dt>Requested at · UTC</dt><dd>{formatUtc(claim.requestedAt)}</dd></div><div><dt>Settled at · UTC</dt><dd>{formatUtc(claim.settledAt)}</dd></div><div><dt>Gross contract need</dt><dd><strong>{formatWireMoney(grossNeed)}</strong></dd></div><div><dt>Platform Fee внутри gross</dt><dd>{formatWireMoney(totalFee)}</dd></div><div><dt>Failure / terminal code</dt><dd className={claim.failureCode ? "is-risk" : ""}>{claim.failureCode || "Нет"}</dd></div></dl>{claim.components.map((component) => <div className="af-claim-component" key={component.type}><div><strong>{componentLabels[component.type]}</strong><span>{component.transferIds.length ? `${component.transferIds.length} transfer link` : "Transfer evidence отсутствует"}</span></div><dl><div><dt>Gross</dt><dd>{formatWireMoney(component.gross)}</dd></div><div><dt>Net</dt><dd>{formatWireMoney(component.net)}</dd></div><div><dt>Fee</dt><dd>{formatWireMoney(component.platformFee)}</dd></div></dl></div>)}<div className="af-method-note">{transferIds.length ? `Связано transfer IDs: ${transferIds.map(shortId).join(", ")}.` : "Полная transfer lineage в demo-источнике отсутствует. Статус не дополняется предположениями интерфейса."}</div></div></aside>;
}
...
export default function AdminFinanceClaims() {
  ...
  const dynamicTabs = [["all", "Все", rows.length], ...statuses.map((item) => [item, item[0].toUpperCase() + item.slice(1), rows.filter((row) => row.status === item).length])];
  ...
  return <div className="af-content af-claims-content">
    ...
    <div className={`af-quality-notice ${meta.partial ? "is-partial" : ""}`}><span><AlertCircle size={13} /></span><p><strong>{meta.partial ? "Частичное покрытие источника." : "Источник сверён."}</strong> Блок {meta.asOfBlockNumber.toLocaleString("en-US")} · {meta.finality} · transfer lineage и claim-delay history неполные.</p><b>{meta.reconciliationStatus}</b></div>
    <div className="af-claims-info"><Info size={20} /><p><strong>Failed остаётся открытым обязательством.</strong> Оно исключается из exposure только после подтверждённого paid, reversed или expired terminal evidence. Platform Fee удерживается внутри gross.</p><button type="button" onClick={() => setMethodologyOpen(true)}>Методика</button></div>
    <section className="af-metrics af-metrics-five"><Metric accent="#ff8716" label="Открытые обязательства" tag={`${openRows.length}`} tagTone="orange" value={formatWireMoney(sumClaims(openRows))} note="Eligible + Requested + Pending + Failed" /><Metric accent="#7a5bb8" label="Maximum Eligible" tag={`${eligibleRows.length}`} tagTone="violet" value={formatWireMoney(sumClaims(eligibleRows))} note="Доступно к запросу на текущем срезе" /><Metric accent="#4e76d0" label="В обработке" tag={`${processingRows.length}`} tagTone="blue" value={formatWireMoney(sumClaims(processingRows))} note="Requested + Pending" /><Metric accent="#cf534c" label="Failed" tag="ACTION" tagTone="red" value={formatWireMoney(sumClaims(failedRows))} note={`${failedRows.length} обязательство остаётся в exposure`} noteTone="risk" /><Metric accent="#239a77" label="Claim delay · median" tag="N/A" tagTone="green" value="N/A" note="Недостаточно paid history для backtest" /></section>

    <div className="af-claims-layout"><section className="af-panel"><PanelHeader title="Реестр claims" subtitle="Lifecycle, gross need и payout components без раскрытия wallet" action={<Tag tone={meta.partial ? "orange" : "green"}>{rows.length} записей</Tag>} /><div className="af-claim-filters"><label><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Claim, participant, cycle или failure code" /></label><select value={cycle} onChange={(event) => setCycle(event.target.value)}><option value="all">Все циклы</option>{[...new Set(rows.map((row) => row.cycleId))].map((id) => <option value={id} key={id}>{shortId(id)}</option>)}</select><select value={age} onChange={(event) => setAge(event.target.value)}><option value="all">Любой возраст</option><option value="fresh">До 24 часов</option><option value="sla">Старше 24 часов</option></select><button type="button" onClick={reset}>Сбросить</button></div><div className="af-claim-totals"><div><span>Principal</span><strong>{formatWireMoney(principalTotal)}</strong></div><div><span>Gross Delta</span><strong>{formatWireMoney(deltaTotal)}</strong></div><div><span>Gross Partner Reward</span><strong>{formatWireMoney(partnerTotal)}</strong></div></div><div className="af-table-scroll"><table className="af-claims-table"><thead><tr><th>Claim / participant</th><th>Состояние</th><th>Cycle ID</th><th>Возраст</th><th className="number">Gross need</th><th className="number">Fee внутри</th><th>Evidence</th></tr></thead><tbody>{filtered.map((row) => <tr className={effectiveSelectedId === row.id ? "selected" : ""} onClick={() => setSelectedId(row.id)} key={row.id}><td><strong>{shortId(row.id)}</strong><small>{shortId(row.participantId)}</small></td><td><Tag tone={statusTone[row.status]}>{row.status.toUpperCase()}</Tag></td><td className="af-lineage">{shortId(row.cycleId)}</td><td className={row.ageHours > 24 && !["paid", "reversed", "expired"].includes(row.status) ? "is-risk" : ""}>{row.ageHours} ч</td><td className="number">{formatWireMoney(row.grossNeed)}</td><td className="number">{formatWireMoney(row.platformFee)}</td><td>{row.transferCount ? `${row.transferCount} transfer` : row.settledAt ? "Terminal code" : "N/A"}</td></tr>)}</tbody></table></div><div className="af-claims-foot">Eligible → Requested → Pending → Paid. Failed не закрывает обязательство; Reversed и Expired требуют terminal evidence.</div></section><ApiClaimDetail request={detailRequest} /></div>
    {methodologyOpen ? <ClaimsMethodology onClose={() => setMethodologyOpen(false)} /> : null}
  </div>;
}
