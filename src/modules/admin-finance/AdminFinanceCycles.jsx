...
function CyclesRequestState({ request }) {
  ...
  const copy = request.status === "loading"
    ? "Получаем lifecycle aggregates из Admin API."
    : request.status === "auth-required"
      ? "API вернул 401. Макетные показатели не подставлены."
      : "Запрос не выполнен. Последние demo-значения намеренно скрыты.";
  return <section className="af-api-boundary" aria-live="polite"><AlertCircle size={24} /><span>CYCLE LIFECYCLE · FAIL-CLOSED</span><h2>{title}</h2><p>{copy}</p>{request.status !== "loading" ? <button type="button" onClick={request.reload}>Повторить запрос</button> : null}<a href="/admin/methodology#gate">Проверить источники и Gate 0</a></section>;
}
...
export default function AdminFinanceCycles() {
  ...
  return <div className="af-content">
    ...
    <section className="af-metrics af-metrics-five">
      ...
      <Metric accent="#239a77" label="Claimable now" tag={claimableCount === null ? "N/A" : `${claimableCount} циклов`} tagTone="green" value={formatWireMoney(claimableNow)} note="Уже eligible; дата фактического claim неизвестна" noteTone="good" />
      <Metric accent="#f6b92f" label="Нагрузка · 7 дней" tag="MAX EXPOSURE" tagTone="yellow" value={formatWireMoney(next7DaysLoad)} note="Provider aggregate; не cash-out forecast" />
      <Metric accent="#cf534c" label="Нагрузка · 30 дней" tag="MAX EXPOSURE" tagTone="red" value={formatWireMoney(next30DaysLoad)} note={`Остающаяся нагрузка ${formatWireMoney(remainingLoad)}`} noteTone="risk" />
    </section>

    <div className="af-cycles-grid">
      <section className="af-panel"><PanelHeader title="Открытие и завершение циклов" subtitle="Количество событий по дням и активный Principal" /><SourceUnavailable title="Lifecycle time series недоступен" copy="Для графика нужны bucketStart, openedCount, closedCount и activePrincipal в одном finalized block cut. Агрегат по продуктам нельзя раскладывать по дням искусственно." /></section>
      <section className="af-panel"><PanelHeader title="Структура открытых циклов" subtitle={`${totalOpen.toLocaleString("ru-RU")} открытых · all-time объём ${formatWireMoney(totalVolume)}`} action={<Tag tone="blue">{ruleVersions} версии</Tag>} /><div className="af-cycle-mix"><div className="af-cycle-mix-top"><div className="af-cycle-donut"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={mix} dataKey="value" innerRadius={38} outerRadius={61} stroke="none" isAnimationActive={false}>{mix.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie></PieChart></ResponsiveContainer><strong>{totalOpen}</strong></div><div className="af-cycle-mix-list">{mix.map((item) => <div key={item.name}><i style={{ background: item.color }} /><span>{item.name}</span><strong>{item.value} · {item.share}</strong></div>)}</div></div><div className="af-rule-note"><strong>Ruleset фиксируется на уровне цикла.</strong><br />Экран не пересчитывает исторические обязательства по текущей ставке.</div></div></section>
    </div>

    <div className="af-cycles-lower">
      <section className="af-panel af-cycle-register">
        <PanelHeader title={apiTabs.find(([id]) => id === activeTab)?.[1] || "Показатели по типам"} subtitle="All-time aggregates по продуктам и версиям правил" action={<Tag tone={meta.partial ? "orange" : "green"}>{meta.partial ? "PARTIAL" : "RECONCILED"}</Tag>} />
        <div className="af-table-scroll"><table><thead><tr><th>Цикл</th><th className="number">Всего</th><th className="number">Открыто</th><th className="number">Закрыто</th><th className="number">Объём</th><th className="number">Claimable now</th><th className="number">7 дней</th><th className="number">30 дней</th><th>Версия</th></tr></thead><tbody>{visibleRows.map((row) => <tr key={row.productKey}><td><strong>{row.label}</strong><small>{row.productKey}</small></td><td className="number">{row.openedCount.toLocaleString("ru-RU")}</td><td className="number">{(Number.isInteger(row.openCount) ? row.openCount : Math.max(0, row.openedCount - row.closedCount)).toLocaleString("ru-RU")}</td><td className="number">{row.closedCount.toLocaleString("ru-RU")}</td><td className="number">{formatWireMoney(row.principal)}</td><td className="number">{formatWireMoney(row.claimableNow)}</td><td className="number">{formatWireMoney(row.next7DaysLoad)}</td><td className="number">{formatWireMoney(row.next30DaysLoad)}</td><td><Tag tone="blue">{row.rulesetVersion}</Tag></td></tr>)}</tbody></table></div>
      </section>
      <section className="af-panel">
        <PanelHeader title="Maximum exposure по продуктам" subtitle="Provider load aggregate; не прогноз даты фактического claim" action={<div className="af-exposure-switch">{[["7d", "7д"], ["30d", "30д"], ["remaining", "Всё"]].map(([id, label]) => <button type="button" className={exposureHorizon === id ? "active" : ""} onClick={() => setExposureHorizon(id)} key={id}>{label}</button>)}</div>} />
        <div className="af-maturity">{visibleRows.map((row) => { const value = Number(atomicDecimal(row[exposureKey])); return <div className="af-maturity-row" key={row.productKey}><span>{row.label}</span><div><i style={{ width: `${Math.max(2, value / maxMaturity * 100)}%`, background: exposureHorizon === "7d" ? "#f6b92f" : exposureHorizon === "30d" ? "#ff8716" : "#503021" }} /></div><strong>{formatWireMoney(row[exposureKey])}</strong></div>; })}<div className="af-method-note">Claimable now показан отдельно. Для cash ladder нужны дата bucket, Principal, Gross Delta, Gross Partner Reward streamed и утверждённый reserve.</div></div>
      </section>
    </div>
    {rulesOpen ? <RulesDialog onClose={() => setRulesOpen(false)} /> : null}
  </div>;
