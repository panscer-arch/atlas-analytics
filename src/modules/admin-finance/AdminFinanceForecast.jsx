...
const horizons = [["24h", "24 часа"], ["7d", "7 дней"], ["30d", "30 дней"], ["90d", "90 дней"]];
...
export default function AdminFinanceForecast() {
  ...
  return <div className="af-content">
    ...
    <section className="af-metrics af-metrics-five">{horizons.map(([id, label], index) => { const item = visibleHorizonMetrics[id]; return <Metric key={id} accent={["#503021","#ff8716","#f6b92f","#4e76d0"][index]} label={`Maximum exposure · ${label}`} tag={item.cycles ? `${item.cycles} циклов` : scenario.toUpperCase()} tagTone={index === 3 ? "blue" : index === 0 ? "brown" : "orange"} value={apiMode ? formatWireMoney(item.value) : formatMoney(Math.round(item.value * scenarioFactor))} note={apiMode ? `${snapshot.scenario} · immutable snapshot` : id === "24h" ? "Principal + Gross Delta + Gross Partner" : id === "7d" ? `${scenario} · eligibility exposure` : id === "30d" ? "Пик eligibility: $17,830 · 18 августа" : "Без гипотетических новых циклов"} />; })}<Metric accent="#cf534c" label="Peak Funding Gap" tag={apiMode ? snapshot?.firstBreachAt ? shortDate(snapshot.firstBreachAt).toUpperCase() : "NO BREACH" : "14 СЕН"} tagTone={hasGap ? "red" : "green"} value={apiMode ? formatWireMoney(peakGap) : "$4,641"} note={hasGap ? "Требуется пополнение до первой даты breach" : "Минимальный резерв не нарушен"} noteTone={hasGap ? "risk" : "good"} /></section>
