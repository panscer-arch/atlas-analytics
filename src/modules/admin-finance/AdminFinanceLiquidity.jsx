...
function ApiLiquidityState({ request }) {
  const title = request.status === "loading" ? "Получаем finalized checkpoint" : request.status === "auth-required" ? "Нужна админ-сессия" : "Источник ликвидности недоступен";
...
function ApiLiquidity({ request }) {
  ...
  return <div className="af-content">
    <div className="af-quality-notice is-partial"><span><Info size={14}/></span><p><strong>Internal Alpha · частичный liquidity checkpoint.</strong> Reported balance имеет собственное время; без archive RPC он не приписывается к finalized block потоков.</p><b>{checkpoint.canonical ? `BLOCK ${checkpoint.asOfBlockNumber.toLocaleString("en-US")}` : "BLOCK N/A"} · {checkpoint.verification?.toUpperCase().replaceAll("_", " ")}</b></div>
    <section className="af-liquidity-metrics"><Metric accent="#503021" label="Reported current balance" tag={checkpoint.canonical?"CANONICAL":"UNVERIFIED"} tone={checkpoint.canonical?"green":"orange"} value={formatApiMoney(summary.reportedClosing)} note={`Источник observedAt ${checkpoint.observedAt || "N/A"}`}/><Metric accent="#4e76d0" label="Canonical checkpoint" tag={checkpoint.canonical?"API":"UNAVAILABLE"} tone={checkpoint.canonical?"green":"orange"} value={formatApiMoney(summary.canonicalClosing)} note="Требует historical balanceOf на том же блоке"/><Metric accent="#239a77" label="Opening balance" tag="UNAVAILABLE" tone="orange" value={formatApiMoney(summary.openingBalance)} note="Cutover balance не утверждён"/><Metric accent="#7a5bb8" label="Restricted amount" tag="NOT SET" tone="orange" value="N/A" note="Политика владельца не утверждена"/><Metric accent="#cf534c" label="Required reserve" tag="NOT SET" tone="orange" value="N/A" note="Порог ликвидности не утверждён"/><Metric accent="#ff8716" label="Residual" tag="UNRECONCILED" tone="red" value={formatApiMoney(summary.residual)} note="Нельзя вычислить без ledger"/></section>
    <section className="af-panel af-alpha-balances"><PanelHeader title="Остатки по контролируемым контрактам" subtitle={`Observed ${new Date(checkpoint.observedAt || meta.generatedAt).toLocaleString("ru-RU", { timeZone: "UTC" })} UTC · ${checkpoint.verification?.replaceAll("_", " ")}`} action={<Tag tone="orange">PARTIAL</Tag>}/><div className="af-table-scroll"><table><thead><tr><th>Контракт</th><th>Адрес</th><th className="number">USDT balance</th></tr></thead><tbody>{data.balances.map((row)=><tr key={row.contractId}><td><strong>{row.label}</strong><small>{row.contractId}</small></td><td><code>{row.maskedAddress}</code></td><td className="number">{formatApiMoney(row.usdt)}</td></tr>)}</tbody></table></div></section>
    <section className="af-panel"><PanelHeader title="Что блокирует полный roll-forward" subtitle="Пробелы источника остаются видимыми до закрытия Gate 0"/><div className="af-method-policy">{meta.partialReasons.map((reason)=><div key={reason}><span>Source gap</span><strong>{reason.replaceAll("_", " ")}</strong></div>)}</div></section>
  </div>;
}
