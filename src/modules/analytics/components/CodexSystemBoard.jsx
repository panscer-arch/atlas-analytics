...
1 Codex pass -> 1 task -> build/check/review`,
  },
  {
    id: "beautiful-design",
    icon: "UI",
    title: "Создай красивый дизайн",
    tag: "Atlas Design",
    readiness: "по запросу",
    summary: "Готовый сценарий, чтобы просить Codex делать красивые Atlas/SuperSUS-страницы: сначала структура, затем верстка, затем проверка desktop/mobile и правки визуальных косяков.",
    prefacts: [
      "Тип результата: вкладка SuperSUS, лендинг, HTML-макет, dashboard, audit page или отдельный блок.",
      "Аудитория: участник, лидер, партнер, команда, инвестор, reviewer или внутренний оператор.",
      "Референс: ссылка, скриншот, test.atlas-system.io, стиль Atlas System или конкретная страница SuperSUS.",
      "Контент: готовый текст, тезисы, таблица, структура блоков или описание того, что нужно объяснить.",
    ],
    useWhen: [
      "Нужно быстро получить красивую страницу, а не сухой текстовый блок.",
      "Есть тема: аудит, прозрачность, whitepaper, региональные лидеры, контент-план, локализация, Security Review.",
      "Нужно, чтобы Codex сам предложил блоки, сверстал, проверил мобильную версию и отдал файл/URL.",
    ],
    steps: [
      "Сказать Codex использовать skill atlas-design-builder и дать тему, аудиторию, формат и референс.",
      "Попросить сначала предложить структуру блоков, если смысл страницы еще не до конца собран.",
      "После согласования попросить сверстать готовую страницу или вкладку в текущем проекте.",
      "Обязательно прогнать browser QA: desktop, mobile, читаемость, отступы, наложения, таблицы и длинный текст.",
      "Попросить исправить найденные визуальные проблемы и только потом дать готовый файл, локальный URL или деплой.",
    ],
    output: [
      "Готовая HTML/React-страница или обновленная вкладка SuperSUS.",
      "Структура блоков: hero, proof/status, таблицы, схемы, FAQ, CTA или рабочая dashboard-зона.",
      "Проверенный desktop/mobile без обрезанного текста, наложений и случайной декоративной каши.",
      "Короткий список, что сделано, где файл и как открыть.",
    ],
    prompt: `Используй skill atlas-design-builder.

Сделай красивую страницу в стиле Atlas System.
Тема: [например: Центр прозрачности / Security Review / Regional Leader Offer]
Формат: [лендинг / вкладка SuperSUS / HTML-макет / dashboard]
Аудитория: [участник / лидер / партнер / команда]
Референс: [ссылка, скриншот или "test.atlas-system.io"]
Язык: русский.
Контент: [вставь текст, тезисы, таблицу или описание смысла]

Что нужно:
1. Сначала коротко предложи структуру блоков.
2. Затем сверстай готовую страницу/блок.
3. Проверь desktop и mobile.
4. Исправь визуальные косяки: отступы, наложения, мобильную версию, читаемость, таблицы.
5. Дай готовый файл, локальный URL или ссылку на деплой.`,
    code: `# Главный skill
atlas-design-builder

# Полезные design-режимы Codex
design-shotgun        # быстро накидать несколько визуальных направлений
design-html           # довести HTML до production-качества
design-review         # найти визуальные ошибки по скрину/браузеру
frontend-app-builder  # собрать полноценный frontend-flow
frontend-testing-debugging # проверить UI, responsive и runtime errors

# Быстрый standalone-макет, если не нужно трогать проект
python3 ~/.codex/skills/atlas-design-builder/scripts/new_html_page.py ./atlas-design-concept.html \\
  --title "Atlas Transparency Center" \\
  --subtitle "Проверяемая архитектура, документы и статус системы в одном интерфейсе."`,
  },
  {
    id: "small-steps",
    icon: "STEP",
