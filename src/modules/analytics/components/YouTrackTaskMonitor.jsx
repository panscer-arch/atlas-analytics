...
const STATUS_FILTERS = [
  ...
  { id: "attention", label: "Нужно уточнение" },
    ...
    <section className="analytics-youtrack">
      ...
      <section className="analytics-youtrack-kpis">
        {[
          ...
          ["Нужно уточнение", summary.attention, summary.attention ? "danger" : "success"],
          ["Ждёт прогера", summary.waitsForDeveloper, summary.waitsForDeveloper ? "danger" : "success"],
      ...
      <section className="analytics-surface analytics-youtrack-statuses">
        <div className="analytics-parser-table-head">
          <div>
            <span className="analytics-kicker">Словарь YouTrack</span>
            <h3>Статусы задач</h3>
          </div>
        </div>
        <div className="analytics-youtrack-status-grid">
          {STATUS_SUMMARY_ORDER.map((status) => (
            <article key={status} className="analytics-youtrack-status-pill">
              <span>{status}</span>
      ...
      {attentionIssues.length ? (
        <section className="analytics-surface analytics-youtrack-attention">
          <div className="analytics-parser-table-head">
            <div>
              <span className="analytics-kicker">Нужно уточнение</span>
              <h3>Задачи, которые требуют уточнения</h3>
            </div>
          </div>
